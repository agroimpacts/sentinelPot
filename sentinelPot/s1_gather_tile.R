...
gather_image <- function(tile_no, config_dir, num_threads = 1) {
  # Load packages
  ...
  logger <- create.logger(
    logfile = file.path(log_dir, glue('s1_gather_tile_{dt}.log')),
    level = "DEBUG"
  )
  ...
  # Collect garbage
  rm(ids, s1_footprints, s1_search_table, bry); gc()
  ...
  if (length(unique(ftps_sub$title)) == length(unique(dates))){
    ...
    suppressMessages(
      bry_raw <- tiles %>% filter(tile == tile_id) %>%
    ...
    clip <- mclapply(title_list, function(title_each) {
      ...
      bg_ratio <- sum(vals == 0 | vals < -30, na.rm = T) / length(vals)
      if (bg_ratio < 0.99) {
        if (bg_ratio > 0.001){ # condition that image has bg
          ...
          suppressMessages(
            pol <- st_multipolygon(
              lapply(st_geometry(pol), function(x) x[1])
    ...
    dates <- unique(dates)
    suppressMessages(
      bry_terra <- tiles %>% filter(tile == tile_id) %>%
        st_buffer(gcs_res * 100) %>% vect()
    )
